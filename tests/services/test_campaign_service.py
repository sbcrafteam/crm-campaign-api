...
def test_create_campaign(mocker, campaign_service):
    mock_campaigns = []
    mocker.patch("app.services.campaign_service.campaigns", mock_campaigns)

    campaign = Campaign(
        name="Test Campaign", start_date="2025-07-01", end_date="2025-07-10"
    )
...
def test_get_campaign_not_found(mocker, campaign_service):
    mocker.patch("app.services.campaign_service.campaigns", [])
